...
#thename = np.append(thename,'pseudo_by_map')
...
param = {'max_depth':5, 'eta':0.05, 'silent':1, 'objective':'binary:logistic','subsample':0.8,'colsample_bytree':0.7,'eval_metric':'auc','nthread':6,'seed':1230}
...
param = {'max_depth':5, 'eta':0.05, 'silent':1, 'objective':'binary:logistic','subsample':0.8,'colsample_bytree':0.7,'eval_metric':'auc','nthread':6}
num_round = 450
...
f = open('XGB_5_005_08_07_450_0112_0446_reg_train_chichi.csv','wb')
for i in range(0,len(pred_train)):
    f.write(str(sample_train_x[i+1,0])+','+str((pred_train[i]))+'\n')
f = open('XGB_5_005_08_07_450_0112_0446_reg_test_chichi.csv','wb')
