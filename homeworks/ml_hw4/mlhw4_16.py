...
y=train[0:120,dimen]
#x_val = np.hstack((np.ones((np.shape(train)[0],1)),train[121:201,0:dimen]))
...
for lamb_i in lamb:
  ...
  y_pred_eval=np.sign(np.dot(x_val,wlin))
  y_pred_test=np.sign(np.dot(x_test,wlin))
  etrain=float(120-np.sum(np.equal(y,y_pred)))/120
  erreval=float(80-np.sum(np.equal(y_val,y_pred_eval)))/80
...
plt.suptitle("Etrain vs lambda")
plt.ylabel('Etrain (rate)')
plt.xlabel('lambda (log10)')
his=plt.scatter(lambo,all_etrain)
plt.show()
